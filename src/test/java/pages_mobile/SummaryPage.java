...
public class SummaryPage extends PageBase {
	...
	// SMRY-ForecastNext24HrsLbl
	// SMRY-ForecastNextRainTile
	...
	public void verifyForecastAverageWindTileNext4HoursLbl() {
		Assert.assertTrue(isDisplayed(forecastAverageWindTileNext4HoursLabelF),
				"Forecast Average Wind Tile Next4Hours Label is not displayed");
	}

	public void verifyForecastAverageWindTileNext4HoursLblText(
			JSONObject jsonData) {
		System.out.println("Sum_currentWindNext the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_currentWindNext"));
		...
		boolean text = text1.equalsIgnoreCase(jsonReader.getValueFromKey(
				jsonData, "Sum_currentWindNext"));
		// Assert.assertTrue(text,
		// "Forecast Average Wind Tile Next4Hours Label text didn't match");
	}

	public void verifyCurrentMaxWindTileRemoteOrLocalLbl() {
		Assert.assertTrue(isDisplayed(currentMaxWindTileRemoteOrLocalLblF),
				"Current Max Wind Tile Remote Label is not displayed");
	}
	...
	public void verifyCurrentMaxWindTileRemoteOrLocalLblText(JSONObject jsonData) {
		...
		if (getText(currentMaxWindTileRemoteOrLocalLblF).length() != 0) {
			text1 = text1.substring(1, text1.length() - 1);
			System.out.println("From the UI -> " + text1);
			System.out.println("Sum_currentWindLocal the Json -> "
					+ jsonReader.getValueFromKey(jsonData,
							"Sum_currentWindLocal"));
			boolean text = text1.equalsIgnoreCase(jsonReader.getValueFromKey(
					jsonData, "Sum_currentWindLocal"));
			// Assert.assertTrue(text,
			// "Current Average Wind Tile Local Label text didn't match");
		} else if (getText(currentMaxWindTileRemoteOrLocalLblF).length() == 0) {
			Assert.assertEquals(text1.length(), 0,
					"Current Average Wind Tile Local Label text didn't match");
		}
	}

	public void verifyCurrentAverageWindTileRemoteOrLocalLbl() {
		Assert.assertTrue(isDisplayed(currentAverageWindTileRemoteOrLocalLblF),
				"Current Average Wind Tile Remote Label is not displayed");
	}

	public void verifyCurrentAverageWindTileRemoteOrLocalLblText(
			JSONObject jsonData) {
		String text1 = getText(currentAverageWindTileRemoteOrLocalLblF);
		text1 = text1.substring(1, text1.length() - 1);
		if (text1.equalsIgnoreCase(jsonReader.getValueFromKey(jsonData,
				"Sum_currentWindRemote"))) {
			System.out.println("From the UI -> " + text1);
			System.out.println("Sum_currentWindRemote the Json -> "
					+ jsonReader.getValueFromKey(jsonData,
							"Sum_currentWindRemote"));
			boolean text = text1.equalsIgnoreCase(jsonReader.getValueFromKey(
					jsonData, "Sum_currentWindRemote"));
			// Assert.assertTrue(text,
			// "Current Average Wind Tile Remote Label text didn't match");
		} else if (text1.equalsIgnoreCase(jsonReader.getValueFromKey(jsonData,
				"Sum_currentWindLocal"))) {
			System.out.println("From the UI -> " + text1);
			System.out.println("Sum_currentWindLocal the Json -> "
					+ jsonReader.getValueFromKey(jsonData,
	...
	public void verifyForecastAverageWindTileLbl() {
		Assert.assertTrue(isDisplayed(forecastAverageWindTileLblF),
				"Forecast Average Wind Tile label is not displayed");
	}

	public void verifyForecastAverageWindTileLblText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(forecastAverageWindTileLblF));
		System.out.println("Sum_currentWind the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_currentWind"));
		boolean text = getText(forecastAverageWindTileLblF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_currentWind"));
		// Assert.assertTrue(text,
		// "Forecast Average Wind Tile label text didn't match");
	}

	public void verifyForecastAverageWindTileValue() {
		Assert.assertTrue(isDisplayed(forecastAverageWindTileValueF),
				"Forecast Average Wind Tile Value is not displayed");
	}
	...
	public void getForecastAverageWindTileValue() {
		...
		if (!Objects.equals(value, "--")) {
			Assert.assertEquals(words[1], speedUnit,
					"Avg Wind units is not equal in API and UI");
		//	Assert.assertEquals(finalValue, Integer.parseInt(words[0]),
		//			"Avg Wind value is not equal in API and UI");
		} else {
			Assert.assertEquals(value, "--", "Avg Wind value is not -- on UI");
			Assert.assertEquals(avgWind, 250,
					"Avg Wind value is not null from API");
		}
	}

	public void verifyForecastAverageWindTile() {
		Assert.assertTrue(isDisplayed(forecastAverageWindTileF),
				"Forecast Average Wind Tile is not displayed");
	}

	public void verifyCurrentMaxWindTileLbl() {
		Assert.assertTrue(isDisplayed(currentMaxWindTileLblF),
				"Current Max Wind Tile Label is not displayed");
	}

	public void verifyCurrentMaxWindTileLblText(JSONObject jsonData) {
		System.out.println("From the UI -> " + getText(currentMaxWindTileValueF));
		System.out.println("max wind from API"+ maxWind);
		// Assert.assertTrue(text,
		// "Current Max Wind Tile Label text didn't match");
	}

	public void verifyCurrentMaxWindTileValue() {
		Assert.assertTrue(isDisplayed(currentMaxWindTileValueF),
				"Current Max Wind Tile Value is not displayed");
	}

	public void verifyCurrentMaxWindTile() {
		Assert.assertTrue(isDisplayed(currentMaxWindTileF),
				"Current Max Wind Tile is not displayed");
	}

	public void verifyCurrentAverageWindTileLbl() {
		Assert.assertTrue(isDisplayed(currentAverageWindTileLblF),
				"Current Average Wind Tile Label is not displayed");
	}

	public void verifyCurrentAverageWindTileLblText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentAverageWindTileLblF));
		System.out.println("Sum_currentWind the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_currentWind"));
		boolean text = getText(currentAverageWindTileLblF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_currentWind"));
		// Assert.assertTrue(text,
		// "Current Average Wind Tile Label text didn't match");
	}

	public void verifyCurrentAverageWindTileValue() {
		Assert.assertTrue(isDisplayed(currentAverageWindTileValueF),
				"Current Average Wind Tile Value is not displayed");
	}

	public void verifyCurrentAverageWindTile() {
		Assert.assertTrue(isDisplayed(currentAverageWindTileF),
				"Current Average Wind Tile is not displayed");
	}

	public void verifyCurrentWindForecastTextField() {
		Assert.assertTrue(isDisplayed(currentWindForecastLabelF),
				"Current Wind Conditions text is not displayed");
	}

	public void verifyCurrentWindForecastText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentWindForecastLabelF));
		System.out.println("Sum_currentWindForecast the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_currentWindForecast"));
		boolean text = getText(currentWindForecastLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_currentWindForecast"));
	...
	public void verifyCurrentWindConditionsTextField() {
		Assert.assertTrue(isDisplayed(currentWindConditionsLabelF),
				"Current Wind Conditions text is not displayed");
	}

	public void verifyCurrentWindConditionsText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentWindConditionsLabelF));
		System.out.println("Sum_currentWindText the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_currentWindText"));
		boolean text = getText(currentWindConditionsLabelF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_currentWindText"));
		// Assert.assertTrue(text, "Current Wind Conditions text didn't match");
	}

	public void verifyBackArrowBtn() {
		Assert.assertTrue(isDisplayed(BackArrowBtnF),
				"Back arrow button is not displayed");
	}
	...
	public void verifyBurgerBtn() {
		Assert.assertTrue(isDisplayed(HamburgerBtnF),
	...
	public void verifyLastSyncLabel(JSONObject jsonData) {
		...
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_lastSync"));
		boolean text = getText(lastSyncLabelF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_lastSync"));
		Assert.assertTrue(text,
				"Summary last sync label default text didn't match");
	}

	public void verifyMarkNumber() {
		Assert.assertTrue(isDisplayed(marksNumberF),
				"Hamburger button is not displayed");
	}
	...
	public void verifyGeolocationName() {
		Assert.assertNotNull(geolocationOfSiteF.getText(),
				"geo location name value is not displayed");
	}

	public void verifySummaryDropdown() {
		Assert.assertTrue(isDisplayed(summaryDropDownF),
				"Summary dropdown is not displayed");
	}

	public void clickSummaryDropdown() {
		Assert.assertTrue(isEnabled(summaryDropDownF),
				"Summary dropdown is not displayed");
	...
	public void verifyDefaultDropdownText(JSONObject jsonData) {
		...
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_dropdownDefault"));
		boolean text = getText(summaryDropDownF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_dropdownDefault"));
	...
	public void getSummaryPageLoadTime() {
		...
		Assert.assertTrue(actualValue < 5,
				"Site summary page loading time is equal or more than 5 sec ");
	}
	...
	public void verifyCurrentConditionsLabel() {
		...
		Assert.assertTrue(isDisplayed(currentConditionsF),
				"Current conditions label is not displayed");
	}

	public void verifyCurrentConditionsText(JSONObject jsonData) {
		System.out.println("From the UI -> " + getText(currentConditionsF));
		System.out
				.println("From the Json -> "
						+ jsonReader.getValueFromKey(jsonData,
								"Sum_currentConditions"));
		boolean text = getText(currentConditionsF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_currentConditions"));
		Assert.assertTrue(text, "Current Conditions text didn't match");
	}

	public void verifyCurrentTemperatureTile() {
		Assert.assertTrue(isDisplayed(currentTemperatureTileF),
				"Current Temperature Tile is not displayed");
	}

	public void verifyCurrentTemperatureTileValue() {
		Assert.assertNotNull(currentTemperatureTileValueF.getText(),
				"Current Temperature Tile value is empty");
	}

	public void verifyCurrentTemperatureTileValueAgainstAPI() {
		System.out.println("Temp Value from UI -> "
				+ currentTemperatureTileValueF.getText());
		Assert.assertNotNull(currentTemperatureTileValueF.getText(),
				"Current Temperature Tile value is empty");
		...
		if (!Objects.equals(value, "--")) {
			...
			Assert.assertEquals(tAir, value1,
					"Temp value is not equal in ui and API");
		} else {
	...
	public void verifyCurrentTemperatureTitleLabel() {
		Assert.assertTrue(isDisplayed(currentTemperatureTileValueF),
				"Current Temperature title label is not displayed");
	}

	public void verifyCurrentTemperatureTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentTemperatureTileLabelF));
		System.out.println("From the API -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_currentTempLabel"));
		boolean text = getText(currentTemperatureTileLabelF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_currentTempLabel"));
		Assert.assertTrue(text,
				"Current Temperature title label text didn't match");
	}

	public void verifyCurrentHumidityTile() {
		Assert.assertTrue(isDisplayed(currentHumidityTileF),
				"Current humidity Tile is not displayed");
	}

	public void verifyCurrentHumidityTileValue() {
		Assert.assertNotNull(currentHumidityTileValueF.getText(),
				"Current humidity Tile value is empty");
	}

	public void verifyCurrentHumidityTileValueAgainstAPI() {
		System.out.println("RH Value from UI -> "
				+ currentHumidityTileValueF.getText());
		Assert.assertNotNull(currentHumidityTileValueF.getText(),
				"Current humidity Tile value is empty");
		...
		if (!Objects.equals(value, "--")) {
			...
			Assert.assertEquals(rh, value1,
					"RH value is not equal in ui and API");
		} else {
	...
	public void verifyCurrentHumidityTitleLabel() {
		Assert.assertTrue(isDisplayed(currentHumidityTileLabelF),
				"Current humidity title label is not displayed");
	}

	public void verifyCurrentHumidityTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentHumidityTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_currentHumidityLabel"));
		boolean text = getText(currentHumidityTileLabelF).equalsIgnoreCase(
				jsonReader
						.getValueFromKey(jsonData, "Sum_currentHumidityLabel"));
		Assert.assertTrue(text,
				"Current humidity title label text didn't match");
	}

	public void verifyCurrentPrecipTile() {
		Assert.assertTrue(isDisplayed(currentPrecipitationTileF),
				"Current Precipitation Tile is not displayed");
	}

	public void verifyCurrentPrecipTileValue() {
		Assert.assertNotNull(currentPrecipitationTileValueF.getText(),
				"Current Precipitation Tile value is empty");
	}

	public void verifyCurrentPrecipTileValueAgainstAPI() {
		System.out.println("Current Conditions UI precip Value is -> "
				+ currentPrecipitationTileValueF.getText());
		Assert.assertNotNull(currentPrecipitationTileValueF.getText(),
				"Current Precipitation Tile value is empty");
		...
		if (!Objects.equals(value, "--")) {
			if (Objects.equals(distanceUnit, "in")) {
				...
				System.out.println("Final UI value used in assertion ->"
						+ Float.parseFloat(value1));
				Assert.assertEquals(precipValue, Float.parseFloat(value1),
						"Precip value is not equal in API and UI");
			} else if (Objects.equals(distanceUnit, "mm")) {
				value1 = stringHelper.trimLastValue(value, 3);
				System.out.println("Not null From the UI -> " + value1);
				System.out.println("Not null From the API ->" + precipValue);
				double doublePrecipValue = Double.parseDouble(String
						.valueOf(precipValue));
				...
				Assert.assertEquals(g, Float.parseFloat(value1),
						"Precip value is not equal in API and UI");
			}
		} else {
			Assert.assertEquals(value, "--", "Precip value is not -- on UI");
			Assert.assertEquals(precipValue, -2.0,
					"Precip value is not null from API");
		}
	}

	public void verifyCurrentPrecipTitleLabel() {
		Assert.assertTrue(isDisplayed(currentPrecipitationTileLabelF),
				"Current Precipitation title label is not displayed");
	}

	public void verifyCurrentPrecipTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentPrecipitationTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader
						.getValueFromKey(jsonData, "Sum_currentPrecipLabel"));
		boolean text = getText(currentPrecipitationTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_currentPrecipLabel"));
		Assert.assertTrue(text,
				"Current Precipitation title label text didn't match");
	}

	public void verifySummaryForecastLabel() {
		Assert.assertTrue(isDisplayed(summaryForecastF),
				"Summary Forecast label is not displayed");
	}

	public void verifySummaryForecastText(JSONObject jsonData) {
		System.out.println("From the UI -> " + getText(summaryForecastF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_forecast"));
		boolean text = getText(summaryForecastF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_forecast"));
		Assert.assertTrue(text, "Summary Forecast text didn't match");
	}

	public void verifyForecastMaxTemperatureTile() {
		Assert.assertTrue(isDisplayed(forecastMaxTemperatureTileF),
				"Forecast Max Temperature Tile is not displayed");
	}

	public void verifyForecastMaxTemperatureTileValue() {
		Assert.assertNotNull(forecastMaxTemperatureTileValueF.getText(),
				"Forecast Max Temperature Tile value is empty");
	}

	public void verifyForecastMaxTemperatureTileValueAgainstAPI() {
		System.out.println("Max Temp Value from UI -> "
				+ forecastMaxTemperatureTileValueF.getText());
		Assert.assertNotNull(forecastMaxTemperatureTileValueF.getText(),
				"Forecast Max Temperature Tile value is empty");
		...
		if (!Objects.equals(value, "--")) {
			...
			Assert.assertEquals(Math.round(maxTemp), Integer.parseInt(value1),
					"Max temp value is not equal in ui and API");
		} else {
	...
	public void verifyForecastMaxTemperatureTitleLabel() {
		Assert.assertTrue(isDisplayed(forecastMaxTemperatureTileLabelF),
				"Forecast Max Temperature title label is not displayed");
	}

	public void verifyForecastMaxTemperatureTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(forecastMaxTemperatureTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_forecastMaxTempLabel"));
		boolean text = getText(forecastMaxTemperatureTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_forecastMaxTempLabel"));
		Assert.assertTrue(text,
				"Forecast Max Temperature title label text didn't match");
	}

	public void verifyForecastMinTemperatureTile() {
		Assert.assertTrue(isDisplayed(forecastMinTemperatureTileF),
				"Forecast Min Temperature Tile is not displayed");
	}

	public void verifyForecastMinTemperatureTileValue() {
		Assert.assertNotNull(forecastMinTemperatureTileValueF.getText(),
				"Forecast Min Temperature Tile value is empty");
	}

	public void verifyForecastMinTemperatureTileValueAgainstAPI() {
		System.out.println("Min Temp Value from UI -> "
				+ forecastMinTemperatureTileValueF.getText());
		Assert.assertNotNull(forecastMinTemperatureTileValueF.getText(),
				"Forecast Max Temperature Tile value is empty");
		...
		if (!Objects.equals(value, "--")) {
			...
			Assert.assertEquals(Math.round(minTemp), Integer.parseInt(value1),
					"Min temp value is not equal in ui and API");
		} else {
	...
	public void verifyForecastMinTemperatureTitleLabel() {
		Assert.assertTrue(isDisplayed(forecastMinTemperatureTileLabelF),
				"Forecast Min Temperature title label is not displayed");
	}

	public void verifyForecastMinTemperatureTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(forecastMinTemperatureTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_forecastMinTempLabel"));
		boolean text = getText(forecastMinTemperatureTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_forecastMinTempLabel"));
		Assert.assertTrue(text,
				"Forecast Min Temperature title label text didn't match");
	}

	public void verifyForecastNxtRainTile() {
		Assert.assertTrue(isDisplayed(forecastNxtRainTileF),
				"Forecast Next Rain Tile is not displayed");
	}
	...
	public void verifyForecastNxtRainTileValue() {
		Assert.assertNotNull(forecastNxtRainTileValueF.getText(),
				"Forecast Next Rain Tile value is empty");
	}

	public void verifyForecastNxtRainTileValueAgainstAPI() {
		if (!forecastNoRainTileLabelF.getText().contains("No Rain Forecasted")) {
			System.out.println("Next rain value from UI -> "
					+ forecastNxtRainTileValueF.getText());
			Assert.assertNotNull(forecastNxtRainTileValueF.getText(),
					"Forecast Next Rain Tile value is empty");
			String value = forecastNxtRainTileValueF.getText();
			String value1;
			if (Objects.equals(PropertiesReader.language, "uk")
					|| Objects.equals(PropertiesReader.language, "de")) {
				value1 = stringHelper.trimLastValue(value, 5);
			} else if (Objects.equals(PropertiesReader.language, "pl")
					|| Objects.equals(PropertiesReader.language, "pt")
					|| Objects.equals(PropertiesReader.language, "fr")) {
			...
			Assert.assertEquals(nextRainfall, Integer.parseInt(value1.trim()),
					"Next Rainfall value is not equal in ui and API");
		}
	}

	public void verifyForecastNxtRainTitleLabel() {
		Assert.assertTrue(isDisplayed(forecastNxtRainTileLabelF),
				"Forecast Next Rain title label is not displayed");
	}

	public void verifyForecastNxtRainTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(forecastNxtRainTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_forecastNxtRainLabel"));
		boolean text = getText(forecastNxtRainTileLabelF).equalsIgnoreCase(
				jsonReader
						.getValueFromKey(jsonData, "Sum_forecastNxtRainLabel"));
		Assert.assertTrue(text,
				"Forecast Next Rain title label text didn't match");
	}

	public void verifyForecastNoRainTitleLabel() {
		Assert.assertTrue(isDisplayed(forecastNoRainTileLabelF),
				"Forecast no Rain title label is not displayed");
	}

	public void verifyForecastNoRainTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(forecastNoRainTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_forecastNoRainLabel"));
		boolean text = getText(forecastNoRainTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_forecastNoRainLabel"));
		Assert.assertTrue(text,
				"Forecast no Rain title label text didn't match");
	}

	public void verifySummaryCropActivityLabel() {
		Assert.assertTrue(isDisplayed(summaryCropActivityF),
				"Summary Crop Activity label is not displayed");
	}

	public void verifySummaryCropActivityText(JSONObject jsonData) {
		System.out.println("From the UI -> " + getText(summaryCropActivityF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_cropActivity"));
		boolean text = getText(summaryCropActivityF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_cropActivity"));
		Assert.assertTrue(text, "Summary CropActivity text didn't match");
	}

	public void verifyCropActivity24HrsTile() {
		Assert.assertTrue(isDisplayed(RecentCropActivity24HrsTileF),
				"Recent CropActivity 24Hrs Tile is not displayed");
	}

	public void verifyCropActivity24HrsTileValue() {
		Assert.assertNotNull(RecentCropActivity24HrsTileValueF.getText(),
				"Recent CropActivity 24Hrs Tile value is empty");
	}

	public void verifyCropActivity24HrsTitleLabel() {
		Assert.assertTrue(isDisplayed(RecentCropActivity24HrsTileLabelF),
				"Forecast Max Temperature title label is not displayed");
	}

	public void verifyCropActivity24HrsTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(RecentCropActivity24HrsTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_cropActivityEtcLabel"));
		boolean text = getText(RecentCropActivity24HrsTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_cropActivityEtcLabel"));
		Assert.assertTrue(text,
				"Crop Activity 24Hrs title label text didn't match");
	}

	public void verifyCropActivity7HrsTile() {
		Assert.assertTrue(isDisplayed(RecentCropActivity7HrsTileF),
				"Crop Activity 7Hrs tile is not displayed");
	}

	public void verifyCropActivity7HrsTileValue() {
		Assert.assertNotNull(RecentCropActivity7HrsTileValueF.getText(),
				"Crop Activity 7Hrs tile value is empty");
	}

	public void verifyCropActivity7HrsTitleLabel() {
		Assert.assertTrue(isDisplayed(RecentCropActivity7HrsTileLabelF),
				"Crop Activity 7Hrs title label is not displayed");
	}

	public void verifyCropActivity7HrsTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(RecentCropActivity7HrsTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_cropActivityEtcLabel"));
		boolean text = getText(RecentCropActivity7HrsTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_cropActivityEtcLabel"));
		Assert.assertTrue(text,
				"Crop Activity 7Hrs title label text didn't match");
	}

	public void verifyCropActivityLeafWetnessTile() {
		Assert.assertTrue(isDisplayed(forecastCropActivityLeafWetnessTileF),
				"Crop Activity Leaf Wetness Tile is not displayed");
	}

	public void verifyCropActivityLeafWetnessTileValue() {
		Assert.assertNotNull(CropActivityLeafWetnessTileValueF.getText(),
				"Crop Activity Leaf Wetness Tile value is empty");
	}

	public void verifyCropActivityLeafWetnessTitleLabel() {
		Assert.assertTrue(isDisplayed(CropActivityLeafWetnessTileLabelF),
				"Crop Activity Leaf Wetness title label is not displayed");
	}

	public void verifyCropActivityLeafWetnessTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(CropActivityLeafWetnessTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_cropActivityLeafWetnessLabel"));
		boolean text = getText(CropActivityLeafWetnessTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_cropActivityLeafWetnessLabel"));
		Assert.assertTrue(text,
				"Crop Activity Leaf Wetness title label text didn't match");
	}

	public void verifySummaryCurrentSeasonLabel() {
		Assert.assertTrue(isDisplayed(summaryCurrentSeasonF),
				"Summary Current Season label is not displayed");
	}
	...
	public void verifySummaryCurrentSeasonText(JSONObject jsonData) {
		...
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData, "Sum_currentSeason"));
		boolean text = getText(summaryCurrentSeasonF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData, "Sum_currentSeason"));
		Assert.assertTrue(text, "Summary Current Season text didn't match");
	}

	public void verifyCurrentSeasonCropTypeTile() {
		Assert.assertTrue(isDisplayed(currentSeasonCropTypeTileF),
				"Recent Current Season CropType Tile is not displayed");
	}

	public void verifyCurrentSeasonCropTypeTileValue() {
		Assert.assertNotNull(currentSeasonCropTypeTileValueF.getText(),
				"Current Season CropType Tile value is empty");
		if (currentSeasonCropTypeTileValueListF.size() == 2) {
			for (int i = 0; i < currentSeasonCropTypeTileValueListF.size(); i++) {
				if (i == 0) {
					Assert.assertNotNull(currentSeasonCropTypeTileValueListF
							.get(i).getText(), "Crop value from UI is not null");
				} else if (i == 1) {
					Assert.assertNotNull(currentSeasonCropTypeTileValueListF
							.get(i).getText(),
							"Crop varietal from UI is not null");
				}
			}
	...
	public void verifyCurrentSeasonCropTypeTileValueWithAPI() {
		Assert.assertNotNull(currentSeasonCropTypeTileValueF.getText(),
				"Current Season CropType Tile value is empty");
		if (currentSeasonCropTypeTileValueListF.size() == 2) {
			String value = restAssuredHelper.getValueFromResponse(response,
					"items[0].crop_varietal.crop_category.name");
			String value1 = restAssuredHelper.getValueFromResponse(response,
					"items[0].crop_varietal.name");
			System.out.println("From the API -> " + value);
			System.out.println("From the API -> " + value1);
			for (int i = 0; i < currentSeasonCropTypeTileValueListF.size(); i++) {
				if (i == 0) {
					System.out.println("From the UI -> "
							+ currentSeasonCropTypeTileValueListF.get(i)
									.getText());
					System.out.println("From the API -> " + value);
					Assert.assertEquals(currentSeasonCropTypeTileValueListF
							.get(i).getText(), value,
							"Crop value from UI and API is not the same");
				} else if (i == 1) {
					System.out.println("From the UI -> "
							+ currentSeasonCropTypeTileValueListF.get(i)
									.getText());
					System.out.println("From the API -> " + value1);
					Assert.assertEquals(currentSeasonCropTypeTileValueListF
							.get(i).getText(), value1,
							"Crop varietal from UI and API is not the same");
				}
			}
		} else if (currentSeasonCropTypeTileValueListF.size() == 1) {
			String value = getText(currentSeasonCropTypeTileValueF);
	...
	public void verifyCurrentSeasonCropTypeTileLabel() {
		Assert.assertTrue(isDisplayed(currentSeasonCropTypeTileLabelF),
				"Recent Current Season CropType tile label is not displayed");
	}

	public void verifyCurrentSeasonCropTypeTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentSeasonCropTypeTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_currentSeasonCropTypeLabel"));
		boolean text = getText(currentSeasonCropTypeTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_currentSeasonCropTypeLabel"));
		Assert.assertTrue(text,
				"Recent Current Season CropType tile label text didn't match");
	}

	public void verifyCurrentSeasonSeasonTile() {
		Assert.assertTrue(isDisplayed(currentSeasonsTileF),
				"Current Season Season tile is not displayed");
	}
	...
	public void verifyCurrentSeasonSeasonTitleLabel() {
		Assert.assertTrue(isDisplayed(currentSeasonsTileLabelF),
				"Current Season Season title label is not displayed");
	}

	public void verifyCurrentSeasonSeasonTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentSeasonsTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_currentSeasonSeasonsLabel"));
		boolean text = getText(currentSeasonsTileLabelF).equalsIgnoreCase(
				jsonReader.getValueFromKey(jsonData,
						"Sum_currentSeasonSeasonsLabel"));
		Assert.assertTrue(text,
				"Current Season Season title label text didn't match");
	}

	public void verifyCurrentSeasonGrowthStageTile() {
		Assert.assertTrue(isDisplayed(currentSeasonGrowthStageTileF),
				"Current Season growth stage Tile is not displayed");
	}

	public void verifyCurrentSeasonGrowthStageTileValue() {
		Assert.assertNotNull(currentSeasonGrowthStageTileValueF.getText(),
				"Current Season growth stage Tile value is empty");
	}

	public void verifyCurrentSeasonGrowthStageTitleLabel() {
		Assert.assertTrue(isDisplayed(currentSeasonGrowthStageTileLabelF),
				"Current Season growth stage title label is not displayed");
	}

	public void verifyCurrentSeasonGrowthStageTitleLabelText(JSONObject jsonData) {
		System.out.println("From the UI -> "
				+ getText(currentSeasonGrowthStageTileLabelF));
		System.out.println("From the Json -> "
				+ jsonReader.getValueFromKey(jsonData,
						"Sum_currentSeasonGrowthStageLabel"));
		boolean text = getText(currentSeasonGrowthStageTileLabelF)
				.equalsIgnoreCase(
						jsonReader.getValueFromKey(jsonData,
								"Sum_currentSeasonGrowthStageLabel"));
		Assert.assertTrue(text,
				"Current Season growth stage title label text didn't match");
	}
