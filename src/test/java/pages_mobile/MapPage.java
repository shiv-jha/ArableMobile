...
public class MapPage extends PageBase {
	...
	public void verifyWelcomeSitesPopup() {
		...
		Assert.assertTrue(isDisplayed(welcomeCloseBthF), "Welcome to sites cancel button is not displayed");
	}
	...
	public void verifyWelcomeSitesCloseText(JSONObject jsonData) {
		if (PropertiesReader.device_type.equalsIgnoreCase("Android")) {
			...
			System.out.println(jsonReader.getValueFromKey(jsonData, "WTS_close"));
			boolean text = welcomeSitesValue.contains(jsonReader.getValueFromKey(jsonData, "WTS_close"));
			Assert.assertTrue(text, "Welcome to sites cancel button text didn't match");
		}
	}

	public void verifyMapDropDown() {
		Assert.assertTrue(isDisplayed(mapDropDownDefaultF), "Welcome to sites cancel button is not displayed");
		clickMapDropDown();
	...
	public void mapDefaultDropDownText(JSONObject jsonData) {
		...
		boolean text = getText(mapDropDownDefaultF)
				.equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "temperature"));
		Assert.assertTrue(text, "Map temperature title text didn't match");
	}

	public void verifyMapShowAllMarkersBtn() {
		Assert.assertTrue(isDisplayed(mapShowMarkersF), "Map all markers button is not displayed");
		Assert.assertTrue(isEnabled(mapShowMarkersF), "Map all markers button is not enabled");
	}

	public void verifyMapCenterBtn() {
		Assert.assertTrue(isDisplayed(mapCenterBtnF), "Map center button is not displayed");
		Assert.assertTrue(isEnabled(mapCenterBtnF), "Map center button is not enabled");
	}

	public void verifyMapMeasureUnits() {
		Assert.assertTrue(isDisplayed(barMeasurementUnitF), "Map measure unit value is not displayed");
	}

	public void verifyMeasuresColorBar() {
		Assert.assertTrue(isDisplayed(measureColourBarF), "measure Colour Bar is not displayed");
	}
	...
	public void verifyUnitsOnMap(String var) {
		...
		if (var.equals("gdd")) {
			Assert.assertTrue(isDisplayed(barMeasurementUnitGDDF), "Measurement units is not displayed");
			Assert.assertNotEquals(initialUnits, updatedUnits, "Measurements are equal before and after");
		} else {
			Assert.assertTrue(isDisplayed(barMeasurementUnitF), "Measurement units is not displayed");
			Assert.assertNotEquals(initialUnits, updatedUnits, "Measurements are equal before and after");
		}
	}
	...
	public void verifyBarUnits(String var, JSONObject jsonData) {
		if (var.equals("temperature")) {
			if (barMeasurementUnitF.getText().equals(jsonReader.getValueFromKey(jsonData, "Settings_fahrenheit"))) {
				Assert.assertEquals(barFirstValueF.getText(), "-10");
				Assert.assertEquals(barLastValueF.getText(), "140");
			} else if (barMeasurementUnitF.getText()
					.equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "Settings_celsius"))) {
			...
			if (barMeasurementUnitF.getText().equals("W/m²")) {
				System.out.println(barFirstValueF.getText());
				System.out.println(barLastValueF.getText());
				Assert.assertEquals(barFirstValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_dark"));
				Assert.assertEquals(barLastValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_bright"));
			} else if (barMeasurementUnitF.getText().equals("Ват/м²")
			...
			System.out.println("Gdd ---> " + barMeasurementUnitGDDF.getText().trim());
			if (barMeasurementUnitGDDF.getText().trim().equalsIgnoreCase("(ºC)")) {
				String[] splitStr = barMeasurementUnitF.getText().split("\\s+");
				System.out.println(splitStr[0]);
				Assert.assertEquals(splitStr[0], jsonReader.getValueFromKey(jsonData, "Map_gdd"));
				Assert.assertEquals(barFirstValueF.getText(), "0");
				Assert.assertEquals(barLastValueF.getText(), "5500");
			} else if (barMeasurementUnitGDDF.getText().trim().equalsIgnoreCase("(ºF)")) {
				String[] splitStr = barMeasurementUnitF.getText().split("\\s+");
				System.out.println(splitStr[0]);
				System.out.println(jsonReader.getValueFromKey(jsonData, "Map_gdd"));
				Assert.assertEquals(splitStr[0], jsonReader.getValueFromKey(jsonData, "Map_gdd"));
				Assert.assertEquals(barFirstValueF.getText(), "0");
				Assert.assertEquals(barLastValueF.getText(), "5500");
			} else {
			...
			if (barMeasurementUnitF.getText().equals("CI")) {
				System.out.println(barFirstValueF.getText());
				System.out.println(barLastValueF.getText());
				Assert.assertEquals(barFirstValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_low"));
				Assert.assertEquals(barLastValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_high"));
			} else {
			...
			if (barMeasurementUnitF.getText().equals("NDVI")) {
				System.out.println(barFirstValueF.getText());
				System.out.println(barLastValueF.getText());
				Assert.assertEquals(barFirstValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_bareEarth"));
				Assert.assertEquals(barLastValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_fullCanopy"));
			} else if (barMeasurementUnitF.getText().equals("НВІР (Нормалізований відносний індекс рослинності)")
					&& PropertiesReader.language.equalsIgnoreCase("uk")) {
				System.out.println(barFirstValueF.getText());
				System.out.println(barLastValueF.getText());
				Assert.assertEquals(barFirstValueF.getText(), jsonReader.getValueFromKey(jsonData, "Map_bareEarth"));
			...
			if (barMeasurementUnitF.getText().equals(jsonReader.getValueFromKey(jsonData, "Settings_mph"))) {
				Assert.assertEquals(barFirstValueF.getText(), "0");
				Assert.assertEquals(barLastValueF.getText(), "80");
			} else if (barMeasurementUnitF.getText().equals(jsonReader.getValueFromKey(jsonData, "Settings_kph"))) {
				Assert.assertEquals(barFirstValueF.getText(), "0");
				Assert.assertEquals(barLastValueF.getText(), "129");
			} else if (barMeasurementUnitF.getText().equals(jsonReader.getValueFromKey(jsonData, "Settings_mps"))) {
				Assert.assertEquals(barFirstValueF.getText(), "0");
			...
			System.out.println("Soil moisture --- " + barMeasurementUnitF.getText());
	...
	public void verifyMapNavigationBtnText(JSONObject jsonData) {
		Assert.assertTrue(isDisplayed(mapNavigationBtnF), "Map navigation button is not displayed");
		Assert.assertTrue(isEnabled(mapNavigationBtnF), "Map navigation button is not enabled");
		System.out.println(getText(mapNavigationBtnF));
		System.out.println(jsonReader.getValueFromKey(jsonData, "Map_barText"));
		boolean text = getText(mapNavigationBtnF).equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "Map_barText"));
		Assert.assertTrue(text, "Map navigation bar title text didn't match");
	}

	public void clickShowMarkAllBtn() {
		Assert.assertTrue(isEnabled(mapShowMarkersF), "Show marks button is not enabled");
		click(mapShowMarkersF);
	}

	public void clickMapCenterBtn() {
		Assert.assertTrue(isEnabled(mapCenterBtnF), "Show map center button is not enabled");
		click(mapCenterBtnF);
	}

	public void verifyGoogleMap() {
		Assert.assertTrue(isDisplayed(googleMapF), "Google map is not displayed");
	}
	...
	public void verifyDropdownHumidityValue() {
		Assert.assertTrue(isDisplayed(mapDropdownHumidityF), "Map dropdown humidity field is displayed");
	}
	...
	public void verifyListInMapDropDown(JSONObject jsonData) {
		...
		if (mapDropdownListF.size() == 0) {
			...
			for (MobileElement value : mapDropdownListF) {
				if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "temperature"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "temperature"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "humidity"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "humidity"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "solar"))) {
					System.out.println(value.getText());
					Assert.assertTrue(true, "Map dropdown value didn't match");
					System.out.println(jsonReader.getValueFromKey(jsonData, "solar"));
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "gdd"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "gdd"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "chlor"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "chlor"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "ndvi"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "ndvi"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "preSince12"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "preSince12"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "preSince10"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "preSince10"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "et"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "et"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "daily"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "daily"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "wind"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "wind"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "soilMoisture"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "soilMoisture"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else if (value.getText().equalsIgnoreCase(jsonReader.getValueFromKey(jsonData, "sprayWindow"))) {
					System.out.println(value.getText());
					System.out.println(jsonReader.getValueFromKey(jsonData, "sprayWindow"));
					Assert.assertTrue(true, "Map dropdown value didn't match");
				} else {
					Assert.fail("Map dropdown options has incorrect value");
				}
	...
	public void verifyListInMapDropDownAndPrint() {
		...
		if (mapDropdownListF.size() == 0) {
			...
			for (MobileElement value : mapDropdownListF) {
					System.out.println("Drop down value: "+value.getText());
			}
		}
	}
